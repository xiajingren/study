...
![image-20210721110012678](README.assets/image-20210721110012678.png)
...
- 从性能上分为 **乐观锁**（基于版本号比对），**悲观锁**

- 从对数据库操作的类型分为 **读锁**，**写锁**（都属于悲观锁）
...
- 从对数据操作的粒度分为 **表锁**，**行锁**
  ...
  - 表锁
    ...
    每次锁住整张表，开销小，加锁快，不会出现死锁；锁粒度大，发生锁冲突的概率最高，并发度最低，一般用在整张表数据迁移的场景

  - 行锁

    每次只锁住数据行，开销大，加锁慢，会出现死锁；锁粒度最小，发生锁冲突的概率最低，并发度最高

![image-20210721140041195](README.assets/image-20210721140041195.png)

- 间隙锁

  ![image-20210721175807002](README.assets/image-20210721175807002.png)

- where 无索引的字段更新行，行锁可能会上升为表锁
...
- 合理设计索引，尽量缩小锁的范围
- 尽可能减小检索条件范围，避免间隙锁
- 尽量控制事务大小，减少锁定资源量和时间长度，涉及事务加锁sql尽量放在事务最后执行
- 尽可能使用低级别事务隔离
...
MVCC 是依赖记录中的 2个隐式字段（DB_TRX_ID-最近修改这条数据的事务ID；DB_ROLL_PTR-回滚指针，指向这条记录的上一个版本），**undo日志**，**Read View（一致性视图）** 来实现的
...
![image-20210722124941023](README.assets/image-20210722124941023.png)
